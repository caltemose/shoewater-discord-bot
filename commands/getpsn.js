...
module.exports = {
	...
	execute: async (message, args, keyv, prefix, guildId) => {
		...
		if (!allPsn) {
			logger(`'${getNameFromMessage(message)}' used 'getpsn' and triggered an error`, 'allPsn not defined', getISOStamp());
			return message.channel.send('Something is really effed up. Tell @shoewater please.');
		}

		const guildPsn = allPsn[guildId];
		if (!guildPsn) {
			logger(`'${getNameFromMessage(message)}' used 'getpsn' and triggered an error`, 'guildPsn not defined', getISOStamp());
			return message.channel.send('The guild\'s PSN list is empty. Tell @shoewater please.');
		}
		
		const allMembers = await keyv.get('members');
		if (!allMembers) {
			logger(`'${getNameFromMessage(message)}' used 'getpsn' and triggered an error`, 'allMembers not defined', getISOStamp());
			return message.channel.send('Something is really effed up. Tell @shoewater please.');
		}
		const guildMembers = allMembers[guildId];
		if (!guildMembers) {
			logger(`'${getNameFromMessage(message)}' used 'getpsn' and triggered an error`, 'guildMembers not defined', getISOStamp());
		...
		if (!arg) {
			logger(`'${getNameFromMessage(message)}' used 'getpsn' without passing a username to lookup.`, getISOStamp());
		...
		if (member) {
			...
			if (!memberPsn) msg = 'Could not find the psn for the given user.';
				...
				logger(`'${getNameFromMessage(message)}' used 'getpsn' and triggered an error`, 'member defined, memberPsn defined but jacked up:', memberPsn, getISOStamp());
			...
			logger(`'${getNameFromMessage(message)}' used 'getpsn' and triggered an error`, 'member not found in members list:', arg, getISOStamp());
