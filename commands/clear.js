...
const { logError, getISOStamp, getNameFromMessage } = require('../helpers/utils');
...
module.exports = {
	...
	execute: async (message, args, keyv, prefix, guildId) => {
		if (!message.member.hasPermission(ADMINISTRATOR)) {
			logError(`'${getNameFromMessage(message)}' tried to access the 'clear' command`, getISOStamp());
		...
		if (!deleteCount || deleteCount < 2 || deleteCount > 100) {
			logError(`'${getNameFromMessage(message)}' used 'clear' command with deleteCount of ${deleteCount}`, getISOStamp());
		...
		message.channel
			.bulkDelete(fetched)
			.catch(error => {
				logError(`bulkDelete error at ${getISOStamp()}`, error);
